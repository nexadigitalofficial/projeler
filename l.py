#!/usr/bin/env python3
"""
drive_to_html.py — Lüks İmmersive HTML Sunum Üretici (v3 + imgbb)
────────────────────────────────────────────────────────────
Google Drive klasöründeki TÜM dosyaları okuyup
premium, sinematik bir HTML sunum sayfası üretir.
Görseller imgbb'ye yüklenir → sunum.html tek başına deploy edilebilir.

Desteklenen dosya türleri:
  • Görseller   : jpg, jpeg, png, gif, webp, svg  → imgbb'ye yüklenir
  • PDF         : canvas tabanlı viewer + sayfa thumbnails  → local
  • Video       : mp4  → local
...
#  AYARLAR
# ════════════════════════════════════════════════════════════
FOLDER_ID        = os.environ.get("FOLDER_ID", "1wl6IORLksewhrWqpCOfjFNgjlC_rAhZT")
...
ASSETS_DIR       = "sunum_assets"   # sadece PDF ve video için

# ── imgbb ────────────────────────────────────────────────────
IMGBB_API_KEY    = os.environ.get("IMGBB_API_KEY", "a441724ad7d0f1ad163ac49f561f8af5")
...
# ════════════════════════════════════════════════════════════
def get_service():
    # Render env variable'larından dosyaları oluştur
    creds_env = os.environ.get("GOOGLE_CREDENTIALS_JSON")
    token_env  = os.environ.get("GOOGLE_TOKEN_JSON")

    if creds_env and not Path(CREDS_FILE).exists():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            raise RuntimeError(
                "Geçerli token bulunamadı!\n"
                "Çözüm: Scripti bir kez LOKAL çalıştır → OAuth ekranını geç → "
                "oluşan token.json içeriğini GOOGLE_TOKEN_JSON env variable olarak Render'a ekle."
            )
...
# ════════════════════════════════════════════════════════════
#  6. ASSET DOSYALARI — imgbb entegrasyonlu
# ════════════════════════════════════════════════════════════
...
def main():
    print("─" * 52)
    print("  drive_to_html.py — Lüks İmmersive HTML Üretici v3")
    print("  imgbb entegrasyonlu — deploy-ready çıktı")
    print("─" * 52)
    ...
    imgbb_cached = len(_imgbb_cache)
    if imgbb_cached:
        print(f"  ℹ  imgbb önbellek: {imgbb_cached} görsel zaten yüklü (atlanacak)")

    print("\n[3/4] Dosyalar işleniyor…")
    stats = cache_stats()
    if stats["files"] > 0:
        print(f"  ℹ  Drive önbellek: {stats['files']} dosya, {stats['size_mb']:.1f} MB  ({CACHE_DIR}/)")
    else:
        print(f"  ℹ  Drive önbellek boş — ilk çalıştırmada tüm dosyalar indirilecek")
    _cache_hits = [0]
    _cache_miss = [0]
    ...
    print(f"\n  Drive önbellek: {_cache_hits[0]} hit (atlandı), {_cache_miss[0]} miss (indirildi)")
    print(f"  imgbb önbellek: {len(_imgbb_cache)} görsel toplam")
  ...
  <div class="tab-content">\n""")
    ...
    print(f"  ✓ {ASSETS_DIR}/ klasörü: {asset_files} dosya (sadece PDF/video)")
    print(f"  ✓ imgbb_cache.json: {len(_imgbb_cache)} görsel URL'si")
    print(f"\n  🚀 sunum.html artık deploy-ready!")
    print(f"     Görseller imgbb'de, PDF'ler {ASSETS_DIR}/ klasöründe.")
    print(f"\n  Lokal test:")
